...
if (!$fileid && $genid) {
    ...
    if ($rec) {
        ...
        if ($task) $fileid = $task->fileid;
    }
}
if (!$fileid) {
    redirect(new moodle_url('/mod/autogenquiz/view.php', ['id' => $id]),
        'Missing file reference. Please re-upload the file.', null, \core\output\notification::NOTIFY_ERROR);
}
...
function render_editable_form(int $id, int $fileid, int $genid, array $cleanjson): void {
    ...
    foreach ($cleanjson as $q) {
        ...
        if (!in_array($ans, ['True','False'], true)) $ans = 'True';
        ...
        foreach (['True','False'] as $opt)
            echo '<option value="'.$opt.'"'.($ans===$opt?' selected':'').'>'.$opt.'</option>';
        echo '</select></div></div>';
        $i++;
    }
    echo '</div><div class="mt-3">';
    echo '<button type="submit" class="btn btn-primary me-2">Save Changes</button>';
    echo '<button type="button" class="btn btn-success" onclick="alert(\'Import to Question Bank (coming soon)\')">Import to Question Bank</button>';
    echo '</div>'.html_writer::end_tag('form');
    echo <<<JS
<script>
function removeQuestion(id){const el=document.getElementById('q_'+id);if(el)el.remove();}
</script>
JS;
}
...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if (!$file) {
        redirect(new moodle_url('/mod/autogenquiz/view.php', ['id'=>$id]),
            'The source file is missing. Please re-upload it.', null, \core\output\notification::NOTIFY_ERROR);
    }

    $task = (object)[
        'fileid'=>$fid,'courseid'=>$course->id,'status'=>'sent_request',
    ...
    if (!is_array($arr) && preg_match('/\[[\s\S]*\]/',$raw,$m)) $arr=json_decode($m[0],true);
    if (!is_array($arr)) {
        echo $OUTPUT->notification('Failed to parse AI output.',\core\output\notification::NOTIFY_ERROR);
        echo '<pre>'.s($raw).'</pre>'.$OUTPUT->footer();exit;
    }

    foreach ($arr as &$q){$q['type']='tf';if(!isset($q['answer']))$q['answer']='True';}
    $gen=(object)[
        'taskid'=>$taskid,'rawtext'=>$file->confirmed_text,'llm_response'=>$res,
        'parsed_response'=>json_encode($arr,JSON_UNESCAPED_UNICODE|JSON_PRETTY_PRINT),
        'is_approved'=>0,'imported_to_bank'=>0
    ];
    $newid=$DB->insert_record('autogenquiz_generated',$gen,true);

    echo html_writer::tag('h5','Generated Questions');
    render_editable_form($id,$fid,$newid,$arr);
...
if ($genid) {
    ...
    if($rec){
        ...
        echo html_writer::tag('h5','Generated Questions');
        render_editable_form($id,$fileid,$rec->id,$arr);
    } else {
